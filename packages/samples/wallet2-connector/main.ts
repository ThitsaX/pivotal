...
const MODULE_ENV_LOCATION = 'packages/samples/wallet2-connector/.env';
...
const bootstrap = async (): Promise<void> => {
    ...
    const {Wallet2ConnectorAppModule} = await import('./app.module');
    const app = await NestFactory.createApplicationContext(Wallet2ConnectorAppModule);
    app.enableShutdownHooks();

    Logger.log('Wallet2 connector is running.', 'Bootstrap');
};
