...
const MODULE_ENV_LOCATION = 'packages/samples/wallet3-connector/.env';
...
const bootstrap = async (): Promise<void> => {
    ...
    const {Wallet3ConnectorAppModule} = await import('./app.module');
    const app = await NestFactory.createApplicationContext(Wallet3ConnectorAppModule);
    app.enableShutdownHooks();

    Logger.log('Wallet3 connector is running.', 'Bootstrap');
};
