...
import {ConnectorSettings, PerformPatchTransfersCommand} from '../../domain';
...
export class ConnectorPatchTransfersListener implements OnModuleInit {
    ...
    static readonly DURABLE_PREFIX = 'connector-consumer-patch-transfers';
    ...
    constructor(
        ...
        this.subject = ConnectorPatchTransfersPublisher.subjectFor(connectorId);
        this.durable = ConnectorPatchTransfersListener.durableFor(connectorId);
    }
    ...
    private static durableFor(connectorId: string): string {
        const suffix = ConnectorPatchTransfersListener.normalizeDurableSegment(connectorId);
        return `${ConnectorPatchTransfersListener.DURABLE_PREFIX}-${suffix}`;
    }
