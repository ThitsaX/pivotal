...
export class ConnectorPostTransfersListener implements OnModuleInit {
    ...
    static readonly DURABLE_PREFIX = 'connector-consumer-post-transfers';
    ...
    constructor(
        ...
        this.subject = ConnectorPostTransfersPublisher.subjectFor(connectorId);
        this.durable = ConnectorPostTransfersListener.durableFor(connectorId);
    }
    ...
    private static durableFor(connectorId: string): string {
        const suffix = ConnectorPostTransfersListener.normalizeDurableSegment(connectorId);
        return `${ConnectorPostTransfersListener.DURABLE_PREFIX}-${suffix}`;
    }
