...
import {WebAuditAppModule} from './app.module';

const ROOT_ENV_LOCATION = '.env';
const MODULE_ENV_LOCATION = 'packages/apps/web-audit/.env';
...
const bootstrap = async (): Promise<void> => {
    ...
    const port = Number(process.env['WEB_AUDIT_PORT'] ?? DEFAULT_HTTP_PORT);

    const app = await NestFactory.create(WebAuditAppModule);
    app.enableShutdownHooks();
    app.use(json({type: ['application/json', 'application/*+json']}));

    await app.listen(port);
    Logger.log(`Web audit is listening on port ${port}.`, 'Bootstrap');
};
