...
const MODULE_ENV_LOCATION = 'packages/apps/web-legacy/.env';
const DEFAULT_HTTP_PORT = 3203;
...
const bootstrap = async (): Promise<void> => {
    ...
    const port = Number(process.env['WEB_LEGACY_PORT'] ?? DEFAULT_HTTP_PORT);
    ...
    const swaggerConfig = new DocumentBuilder()
        ...
        .setDescription(
            'Legacy API for initiating FSPIOP lookup, quoting, and transfer flows.',
        )
        .setVersion('1.0.0')
        .addBearerAuth(
            {
    ...
    Logger.log(`Web legacy is listening on port ${port}.`, 'Bootstrap');
};
