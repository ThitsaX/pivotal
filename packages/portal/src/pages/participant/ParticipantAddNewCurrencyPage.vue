...
const viewDefinition = VIEW_BY_KEY['participant-add-new-currency'];
...
const canSubmit = computed((): boolean => {
    return form.name.trim().length > 0 && !loading.value;
});
...
const submit = async (): Promise<void> => {
    ...
    try {
        ...
        successMessage.value = `Currencies ${requestPreview.value.currencies.join(', ')} were added for ${form.name.trim()}.`;
    } catch (error) {
