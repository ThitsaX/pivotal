# Services and Gateways — KMS-Backed

Wiring the platform to the certificate authorities you built in
[`1-ca-ceremony.md`](./1-ca-ceremony.md): Vault auth roles per workload, cert-manager, MCM,
trust-manager, and both gateways.

Run **once per environment**, after the ceremony and before onboarding any DFSP.
...
```bash
# web-outbound: signs for every tenant it fronts, so it reads the whole prefix
vault policy write pivotal-jws-key-read-all - <<'EOF'
path "pivotal-kv/data/pivotal/jwskey/*" { capabilities = ["read"] }
path "sys/internal/ui/mounts/pivotal-kv" { capabilities = ["read"] }
EOF

# each connector: reads exactly ONE key path, its own
vault policy write pivotal-jws-key-read-demodfsp1 - <<'EOF'
path "pivotal-kv/data/pivotal/jwskey/DemoDFSP1" { capabilities = ["read"] }
path "sys/internal/ui/mounts/pivotal-kv" { capabilities = ["read"] }
EOF

# web-pivotal: signs DFSP CSRs, and provisions tenant signing keys
vault policy write pivotal-dfsp-sign - <<'EOF'
path "pki_dfsp/sign/dfsp-client" { capabilities = ["create", "update"] }
path "pivotal-kv/data/pivotal/jwskey/*" { capabilities = ["create", "read", "update"] }
path "sys/internal/ui/mounts/pivotal-kv" { capabilities = ["read"] }
EOF
...
> **Give each connector its own policy and its own key path.** A shared wildcard means compromising
> one connector hands over every tenant's signing key.
...
**Next:** [`3-turn-it-on-and-verify.md`](./3-turn-it-on-and-verify.md) — the environment variables, the order to switch the controls on in, and how to prove each one works.
