...
# Where the SoftHSM token store and the public output land. Deliberately outside the
# repository: the token store holds the rehearsal root keys, and nothing here should be
# one `git add -A` away from being committed. Override with CEREMONY_DIR.
HERE=${CEREMONY_DIR:-${TMPDIR:-/tmp}/pivotal-ceremony}
