# Turn It On and Verify — HSM-Backed
...
actually working, and what goes wrong. The only setting that differs between backends is
`PKCS11_MODULE_PATH`.

Run after [`3-services-and-gateways.md`](./3-services-and-gateways.md), and again whenever you need
to check an environment is still sound.

> ## `KEY_PROVIDER=pkcs11` is not implemented
>
> `Pkcs11JwsKeyProvisioner` throws, and the Java side has only `Static` and `Vault` key providers.
> Setting `KEY_PROVIDER=pkcs11` today fails at startup on the read side and at provisioning time on
> the write side — deliberately, because a silent fall back to a software key in a deployment that
> chose hardware custody would be worse.
>
> So this document describes the target state. Until the provider lands, an HSM-backed environment
> runs its services on `vault-kv` while the cluster, the crypto users and the ceremony are proved
> separately.

> **The env-var table below carries three settings this profile needs and the KMS one does not** —
> the PKCS#11 module path, the cluster certificate, and the per-workload crypto-user path. Everything
> else in this document is identical to
> [`../../kms/setup/3-turn-it-on-and-verify.md`](../../kms/setup/3-turn-it-on-and-verify.md); change
> both together.
...
redeploy to rotate, and leaves no record of who read it.
...
| --- | --- | --- |
| `KEY_PROVIDER` | `pkcs11` | Signs inside CloudHSM. Vault holds a crypto-user credential and a keyRef, never a private key |
...
| `VAULT_ROLE` | per workload | Must match the Vault role created in [`3-services-and-gateways.md`](./3-services-and-gateways.md) step 1 |
...
| --- | --- |
| `certificate_required` / TLS alert 116 | The caller presented no client certificate. Check `FSPIOP_USE_MUTUAL_TLS`, the mounted cert paths, and — for a Java connector — the entrypoint mapping check in [`onboard-dfsp.md`](../runbooks/onboard-dfsp.md) step 6 |
