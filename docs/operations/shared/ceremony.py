...
Two subcommands:
  ...
  sign-crl            builds a root-signed CRL — needed to revoke an intermediate
...
from asn1crypto import algos, core, crl as asn1crl, csr as asn1csr, keys, pem, x509
...
def build_crl(signer, root_pem: bytes, revoked: list, days: int, number: int) -> bytes:
    """Build a CRL signed by the root.

    Revoking an intermediate needs a root-signed CRL, and the root key only exists inside
    KMS -- so the same build-here-sign-there split used for certificates applies. Produce
    one during the ceremony even with nothing revoked: an empty CRL proves the path works
    while the tooling is open, which is far cheaper than discovering it does not during an
    incident.

    `revoked` is a list of (serial_int, iso8601_date) pairs. Empty is valid and usual.
    """
    root = x509.Certificate.load(strip_pem(root_pem))
    verify_signer_matches_root(signer, root)
    ...
    entries = asn1crl.RevokedCertificates(
        [
            asn1crl.RevokedCertificate(
                {
                    "user_certificate": serial,
                    # asn1crypto refuses a naive datetime, and a bare YYYY-MM-DD parses
                    # as one -- so force UTC rather than letting it raise at ceremony time.
                    "revocation_date": x509.Time(
                        {
                            "utc_time": datetime.fromisoformat(when).replace(
                                tzinfo=timezone.utc
                            )
                        }
                    ),
    ...
    tbs = asn1crl.TbsCertList(
        {
            "version": "v2",
            "signature": algos.SignedDigestAlgorithm({"algorithm": SIGNATURE_ALGORITHM}),
            "issuer": root.subject,
            "this_update": x509.Time({"utc_time": now}),
            "next_update": x509.Time({"utc_time": now + timedelta(days=days)}),
            "revoked_certificates": entries,
            "crl_extensions": asn1crl.TBSCertListExtensions(
                [
                    asn1crl.TBSCertListExtension(
                        {"extn_id": "crl_number", "extn_value": number}
                    ),
                    asn1crl.TBSCertListExtension(
                        {
                            "extn_id": "authority_key_identifier",
                            "extn_value": x509.AuthorityKeyIdentifier(
                                {"key_identifier": key_identifier(root.public_key)}
                            ),
                        }
                    ),
                ]
    ...
    certificate_list = asn1crl.CertificateList(
        {
            "tbs_cert_list": tbs,
            "signature_algorithm": algos.SignedDigestAlgorithm(
                {"algorithm": SIGNATURE_ALGORITHM}
            ),
            "signature": core.OctetBitString(signature),
        }
    )
...
def main(argv=None) -> int:
    ...
    crl_cmd = subcommands.add_parser("sign-crl")
    crl_cmd.add_argument("--kms-key", required=True)
    crl_cmd.add_argument("--root-cert", required=True)
    crl_cmd.add_argument(
        "--revoked",
        default="",
        help="comma-separated serial:YYYY-MM-DD pairs; empty for an empty CRL",
    )
    crl_cmd.add_argument("--days", type=int, default=180)
    crl_cmd.add_argument("--number", type=int, default=1, help="CRL number, must increase")
    crl_cmd.add_argument("--out", required=True)
    ...
    elif args.command == "sign-crl":
        revoked = [
            (int(part.split(":")[0]), part.split(":", 1)[1])
            for part in args.revoked.split(",")
            if part.strip()
        ]
        certificate = build_crl(
            signer, read(args.root_cert), revoked, args.days, args.number
        )
    else:
