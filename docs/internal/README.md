# Trust-Manager — Design and Status

**Internal.** Planning, architecture and progress. Operators do not read this — their guides are in [`../operations/`](../operations/).
...
```
docs/internal/         ← you are here · planner and architect only
...
├── evidence/          what was proved, and when
└── external/          written for people outside Pivotal

docs/operations/       ← operators and DevOps · no planning content reaches it
│   ── one folder per deployment profile; each holds its own setup/ and runbooks/,
│      and every guide keeps its scripts in a scripts/ folder beside it ──
├── hsm/               HSM-backed  · KEY_PROVIDER=pkcs11 · CloudHSM (or SoftHSM)
├── kms/               KMS-backed  · KEY_PROVIDER=vault-kv · AWS KMS roots
├── local/             development and CI · SoftHSM2
└── shared/            signer-agnostic certificate builder (the KMS ceremony uses it)

setup/    = ONE-TIME, per environment
runbooks/ = repeated, run again every time
```

**The split is deliberate.** Everything under `operations/` is written for someone bringing up an
environment who has no knowledge of this folder — no decision numbers, no requirement labels, no
status. Keep it that way: a planning reference that leaks into a runbook sends an operator looking
for a document they cannot read.
...
### hsm/ · kms/ · local/ — one folder per profile

Each profile folder is **self-contained**: its own `setup/` (numbered, run in order, once) and its
own `runbooks/` (named by trigger, run again every time). Every guide keeps its scripts in a
`scripts/` folder beside it, so a document can name the exact path to run.

There is no shared onboarding document. The two profiles diverge enough that a single one meant
readers following a trail out of their own folder — so each profile carries the whole procedure, and
each says at the top which steps are shared with the other.

**[`hsm/`](../operations/hsm/) — HSM-backed, `KEY_PROVIDER=pkcs11`** · [`README.md`](../operations/hsm/README.md)

| | Document |
| --- | --- |
| setup | [`1-cloudhsm-cluster.md`](../operations/hsm/setup/1-cloudhsm-cluster.md) · [`2-ca-ceremony.md`](../operations/hsm/setup/2-ca-ceremony.md) · [`3-services-and-gateways.md`](../operations/hsm/setup/3-services-and-gateways.md) · [`4-turn-it-on-and-verify.md`](../operations/hsm/setup/4-turn-it-on-and-verify.md) |
| runbooks | [`onboard-dfsp.md`](../operations/hsm/runbooks/onboard-dfsp.md) · [`rotate-signing-key.md`](../operations/hsm/runbooks/rotate-signing-key.md) · [`offboard-dfsp.md`](../operations/hsm/runbooks/offboard-dfsp.md) |

**[`kms/`](../operations/kms/) — KMS-backed, `KEY_PROVIDER=vault-kv`** · [`README.md`](../operations/kms/README.md) — what
every environment runs today

| | Document |
| --- | --- |
| setup | [`1-ca-ceremony.md`](../operations/kms/setup/1-ca-ceremony.md) · [`2-services-and-gateways.md`](../operations/kms/setup/2-services-and-gateways.md) · [`3-turn-it-on-and-verify.md`](../operations/kms/setup/3-turn-it-on-and-verify.md) |
| runbooks | [`onboard-dfsp.md`](../operations/kms/runbooks/onboard-dfsp.md) · [`rotate-signing-key.md`](../operations/kms/runbooks/rotate-signing-key.md) · [`offboard-dfsp.md`](../operations/kms/runbooks/offboard-dfsp.md) |

**[`local/`](../operations/local/) — development and CI, SoftHSM2** · [`README.md`](../operations/local/README.md)

| | Document |
| --- | --- |
| setup | [`ceremony.md`](../operations/local/setup/ceremony.md) — the same ceremony against SoftHSM2 in Docker. The PKCS#11 code path, so it genuinely rehearses `hsm/` — but it has no crypto-user model, so it cannot rehearse ownership or sharing |
| runbooks | [`reseed-signing-keys.md`](../operations/local/runbooks/reseed-signing-keys.md) — the dev-mode Vault pod restarted and lost every key |

### shared/ — the signer-agnostic certificate builder

[`ceremony.py`](../operations/shared/ceremony.py) builds the X.509 certificates for a CA ceremony — root,
intermediate and CRL — and asks an external signer to sign each one. Neither AWS KMS nor CloudHSM is
a certificate authority; both only sign a digest, so something has to construct the certificate
around it.

**The KMS ceremony calls it; the HSM and local ceremonies build their certificates with OpenSSL
through the PKCS#11 engine instead.** It sits here rather than inside `kms/` because it is written to
be signer-agnostic and the HSM ceremony could adopt it by swapping one call — see
[`shared/README.md`](../operations/shared/README.md).
...
| 24 | **No service holds a Crypto Officer credential — a custodian creates each crypto user** | **Amended 2026-09-20; the original gave the credential to trust-manager so onboarding stayed one portal click.** R7 requires separation of duties naming key management, and R8 a maker-checker model; a single service holding both user administration and key generation is what R7 exists to prevent. So the Crypto Officer credential stays with named custodians and enters no service, no environment variable and not Vault. A custodian creates `cu-<fspId>`; **generation and sharing stay automated**, because both are crypto-user operations — the cost is one short human step per onboarding and per offboarding, on a path that already has a manual prerequisite in MCM registration. **What this closes:** trust-manager can no longer create a crypto user or reset a password, so it cannot mint an identity or take over a tenant it was never given. **What remains, stated plainly:** it reads every tenant's CU credential from Vault in order to generate, so it can still authenticate as any tenant it has provisioned. That is covered by Vault's own audit — a read of `hsmcred/*` outside a provisioning window is the anomaly to alarm on — together with the alarm on Crypto Officer operations. `cu-trust-manager` is **not created**: nothing authenticates as it, so it and its zero-threshold `C_Sign` alarm would watch nothing. Resolves open decision **O** — `architecture.md` §4.3 |
| 25 | **Intermediate-CA rotation is a runbook, not a built feature** | Revoking an intermediate alone strands the system — every leaf under it becomes untrusted and nothing reissues them — so the real operation is *rotate*, with revocation last: new intermediate, reissue leaves, distribute **both** anchors, then remove the old one. That is a five-stage workflow whose two hardest stages cannot be automated anyway: signing the new intermediate needs the root key no workload may reach, and re-enrolling DFSP leaves is one CSR exchange per DFSP. Deferred because the event is decision-triggered rather than clock-driven — the ceremony issues intermediates with a **5-year** validity — and every mechanical piece already exists (ceremony script, Vault `set-signed`, `DfspCaPublishScheduler`, cert-manager). The price is two obligations: write the runbook now, including the anchor-overlap rule, and put the expiry somewhere that will shout. Resolves open decision **P** |
| 26 | **CA roots in hardware, intermediates in Vault — confirmed against the requirements, not assumed** | **R6** is the requirement governing PKI and it never mentions HSMs, while **R5** says "within the HSM" explicitly about the key lifecycle — so the omission in R6 is a choice. **R1** closes by deferring the treatment of *application-layer keys* to the inception phase, which makes stating the boundary expected rather than presumptuous. The boundary taken: application-layer keys are the per-DFSP JWS signing keys and go in CloudHSM; CA keys are PKI infrastructure, roots in hardware, intermediates in Vault. **The revocability argument is explicitly not the basis** — nothing in the deployment checks a CRL and no PKI role sets `crl_distribution_points`; the load is carried by blast radius, Vault's access control and audit, short leaf validity, and the `participant_cert` fingerprint binding. Resolves open decision **L** — `architecture.md` §4.0 |
