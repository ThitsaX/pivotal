...
### What `DFSP_FACING_MTLS_MANDATORY` actually controls
...
So the checks in §4 key on **XFCC presence, per request**. `DFSP_FACING_MTLS_MANDATORY=true` means
only that **a request arriving without XFCC is rejected** — the final hardening step, flipped once
the last DFSP has migrated and the legacy endpoint is gone. See [`architecture.md`](./architecture.md) §6.1.
...
to zero is the signal that the legacy endpoint can be retired and
`DFSP_FACING_MTLS_MANDATORY=true` set — the same role the "which key verified" log plays for
accessKey rotation.
