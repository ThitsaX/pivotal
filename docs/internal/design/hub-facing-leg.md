...
and sharing between CUs. That is what makes "a compromised connector signs as one DFSP" true. The
provisioning model is settled — decision 24: trust-manager creates each tenant CU and generates as
it, with the alarm on Crypto Officer operations.
