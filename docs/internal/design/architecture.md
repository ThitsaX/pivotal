...
| **R5** | A **complete key lifecycle** — generation, distribution, rotation, revocation and **destruction** — all performed within the HSM |
| **R6** | **PKI for certificate-based participant authentication** — issuance, renewal and revocation, with **revocation status verifiable at the time of authentication**, and a unique certificate per participant with none shared across institutions |
| **R7** | **Separation of duties** for all sensitive functions, naming key management among them |
| **R8** | A **maker-checker** approval model for all hub operations and critical configuration changes |

**R5 names an operation this design does not yet have.** Generation, distribution and rotation map
onto `C_GenerateKeyPair`, CU sharing and `rotate-jws-key`. **Destruction has nothing behind it** —
`C_DestroyObject` is small, but it is mandatory and currently absent.

**R6 is the requirement that governs PKI, and it says nothing about HSMs** — which is what settles
open decision **L**; see decision 26. It also states a control Pivotal already satisfies:
`DfspCertificateGuard` reads `participant_cert` on every request, so revocation status is verified at
authentication time rather than from a cached list. What R6 does expose is **renewal**, which is
manual today with no expiry alerting and no `participant_contact`.

**R7 and R8 bear on decision 24** — one service holding both crypto-officer and key-generation
powers is the shape separation of duties exists to prevent, and a single portal click is not
maker-checker.
...
| ~~`cu-trust-manager`~~ — **not created**, decision 24 | — | — | — |
...
**No service holds a Crypto Officer credential** — decision 24. A custodian creates each tenant's
crypto user; the provisioning script writes that user's own password to Vault, and trust-manager
reads it there to generate. So trust-manager cannot create a crypto user or reset one's password,
and cannot mint an identity or take over a tenant it was never given.

**What it can still do, which must not be described as closed:** it reads every tenant's CU
credential in order to generate that tenant's key, so **it can authenticate as any tenant it has
provisioned and sign as that DFSP.** The ownership rule above makes this unavoidable — whoever
generates must hold the owner's credential. Rotating a CU password does not change it, because
trust-manager would generate the replacement too.
...
**Compensate with detection, not with structure — and alarm on the right operations.** Two alarms
carry what the structure above cannot.

**Crypto Officer operations** — `user create`, `user change-password`. A Crypto Officer can reset any
crypto user's password, after which a signature made as `cu-<fspId>` is indistinguishable from that
connector's ordinary traffic. Under decision 24 these operations happen only inside a scheduled
custodian session, so any occurrence outside one is the anomaly.

**Vault reads of `hsmcred/*`.** This is the alarm that covers the residual above. trust-manager
legitimately reads a tenant's crypto-user credential when provisioning or rotating that tenant's key,
and at no other time — so a read outside such a window is the signal that it is being used to sign
rather than to generate. Vault records every read, which is why the compensating control can live
there once the HSM can no longer provide it.

An earlier version of this section alarmed on `C_Sign` by `cu-trust-manager`. **That alarm is
retired**, along with the user: under decision 24 nothing ever authenticates as `cu-trust-manager`,
so it and its alarm would watch nothing.
...
- **CloudHSM authenticates with crypto-user credentials, not IAM policy**, and those credentials are
  static. Provisioning, rotation and the ownership matrix above are all settled — decision 24, which
  resolved **O**. Rotation is an operational runbook rather than a schedule, and it does not reduce
  the residual privilege, because trust-manager generates the replacement too.
...
| --- | --- | --- |
| `DFSP_FACING_MTLS_MANDATORY` | `true` / `false` | **Requires** a client certificate on every DFSP-facing request. A hardening switch flipped *after* migration completes — **not** the mechanism that enables mTLS. See §6.1 |
...
### 6.1 `DFSP_FACING_MTLS_MANDATORY` does not gate the certificate checks
...
`DFSP_FACING_MTLS_MANDATORY=true` then means one thing: **a request without XFCC is rejected.** It
is the final hardening step, flipped once the last DFSP has migrated and the legacy endpoint has been
retired.
