...
> **Corrected 2026-09-08.** The guard used to treat `DFSP_FACING_MTLS_MANDATORY` as a global switch: off meant
...
| Where | Change |
| --- | --- |
| `local/setup/scripts/setup-vault-pki.sh` | the `dfsp-client` role sets `use_csr_common_name=false use_csr_sans=false` — SANs are the other place an identity can hide |
