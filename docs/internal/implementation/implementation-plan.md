...
[`architecture.md`](../design/architecture.md) §3: any connector could then sign as any DFSP.

**Provisioning is settled — decision 24.** **No service holds a Crypto Officer credential.** It stays
with named custodians, who create `cu-<fspId>` in a scheduled session; the provisioning script then
writes that crypto user's own password to Vault for trust-manager to read. Generation and sharing
remain automated, because both are crypto-user operations — only creating the user needs a Crypto
Officer.

Two alarms carry what structure cannot. **Crypto Officer operations** (`user create`,
`user change-password`) should occur only inside a scheduled custodian session, and a Crypto Officer
can reset any CU's password — after which a signature made as `cu-<fspId>` is indistinguishable from
that connector's own traffic, so this alarm is the only thing that would catch it. And **Vault reads
of `hsmcred/*`**, because trust-manager necessarily holds every tenant's CU credential in order to
generate: a read outside a provisioning or rotation window is the anomaly.

Rotation of a CU password is an operational runbook with no expiry forcing it.
...
- **Alarm on `kms:Sign` for the root key.** Legitimate use is roughly three events for the life of
  ...
  pattern as the Crypto Officer alarm in [`architecture.md`](../design/architecture.md) §4.3. Normal Pivotal and
...
increments the no-certificate counter — neither behaviour depends on
`DFSP_FACING_MTLS_MANDATORY`, which is set only after the legacy endpoint is retired.
