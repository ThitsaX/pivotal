...
[`kms/runbooks/onboard-dfsp.md`](../../operations/kms/runbooks/onboard-dfsp.md)** (or the `hsm/` one) — the operator steps,
...
[`kms/runbooks/onboard-dfsp.md`](../../operations/kms/runbooks/onboard-dfsp.md).
...
4. **Run the ceremony for `dev2`, and re-run `local/setup/scripts/setup-vault-pki.sh` in every provisioned
...
`local/setup/ceremony.md` remain outstanding; the runbook predates the profile rename. SoftHSM2
...
| # | JWS | mTLS | Overall |
| --- | --- | --- | --- |
| 1 | 🟢 accessKey custody unchanged (DFSP-held), and proven end to end | 🟢 **LIVE in dev2** — gateway `MUTUAL`, `DFSP_FACING_MTLS_MANDATORY=true`, impersonation refused | **done** |
...
- ~~**The flag is all-or-nothing per deployment**~~ — **fixed 2026-09-08.** `DfspCertificateGuard`
  used to return immediately when this flag was off, so nobody was checked even when they
  ...
  The guard now keys on **XFCC presence per request**, and `DFSP_FACING_MTLS_MANDATORY` decides
  only whether XFCC is *mandatory*:
  ...
  **Renamed 2026-09-10**, `DFSP_FACING_MTLS` → `DFSP_FACING_MTLS_MANDATORY`. The old name read as a
  switch that turned mutual TLS on and off, which is what the 2026-09-08 behaviour was corrected
  away from; readers kept inferring the old meaning from it. The new name states what the flag
  decides — whether an absent certificate is fatal — and matches the guard's own `mandatory` field.
  Behaviour is unchanged. No fallback to the old name: it was never released to production, and
  only dev2 set it.
...
| Local dev environment | 🟢 | k3d + Vault + cert-manager + **SoftHSM2** all in place, each with a re-runnable script. `local/setup/ceremony.md` is now implemented by `local/setup/scripts/ceremony-local.sh` — the runbook prose is still pre-rename and single-domain, but the script supersedes it |
