...
import { EnvBasedPublicKeyStore } from '../../../../../../packages/shared/security/component/key/store/env-based-public-key-store';
...
describe('EnvBasedPublicKeyStore', () => {
    ...
    it('should throw when expected key variable is missing', () => {
        ...
        delete process.env['FSPIOP_JWS_PUBLIC_KEY_FSP-A'];
        const store = new EnvBasedPublicKeyStore();

        assert.throws(
            () => store.load(),
            /Missing public key for 'fsp-a'. Expected env var 'FSPIOP_JWS_PUBLIC_KEY_FSP-A'./,
        );
    });
});
