...
describe('OutboundQuotesAuditPublisher (integration)', () => {

    let nats: NatsClientService;
    let publisher: OutboundQuotesAuditPublisher;
    let db: Client;
    ...
    before(async () => {
        ...
        publisher = new OutboundQuotesAuditPublisher(nats);
    ...
    const findRow = (id: string) =>
        db
            .query<Record<string, unknown>>('SELECT * FROM outbound_quotes WHERE id = $1', [id])
            .then(r => r.rows[0]);

    // ── tests ─────────────────────────────────────────────────────────────────

    it('should publish request stage — consumer inserts row with no response or error', async () => {
        const input = new AuditOutboundQuotesCommand.Input(
            ID_1,
            'FSPIOP',
            'payer-fsp',
            'payee-fsp',
