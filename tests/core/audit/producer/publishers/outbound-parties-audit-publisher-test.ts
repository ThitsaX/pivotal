...
describe('OutboundPartiesAuditPublisher (integration)', () => {

    let nats: NatsClientService;
    let publisher: OutboundPartiesAuditPublisher;
    let db: Client;
    ...
    before(async () => {
        ...
        publisher = new OutboundPartiesAuditPublisher(nats);
    ...
    const findRow = (id: string) =>
        db
            .query<Record<string, unknown>>('SELECT * FROM outbound_parties WHERE id = $1', [id])
            .then(r => r.rows[0]);
