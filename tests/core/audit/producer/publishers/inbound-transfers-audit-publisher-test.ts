...
describe('InboundTransfersAuditPublisher (integration)', () => {

    let nats: NatsClientService;
    let publisher: InboundTransfersAuditPublisher;
    let db: Client;
    ...
    before(async () => {
        ...
        publisher = new InboundTransfersAuditPublisher(nats);
    ...
    const findRow = (id: string) =>
        db
            .query<Record<string, unknown>>('SELECT * FROM inbound_transfers WHERE id = $1', [id])
            .then(r => r.rows[0]);

    // ── tests ─────────────────────────────────────────────────────────────────

    it('should publish request stage — consumer inserts row with no response, error or fspError', async () => {
        const input = new AuditInboundTransfersCommand.Input(
            ID_1,
            'FSPIOP',
            'payer-fsp',
            'payee-fsp',
    ...
    it('should publish fspError stage — consumer inserts row with fsp_error and failed=true', async () => {
        const input = new AuditInboundTransfersCommand.Input(
            ...
            null,
            null,
            'Transfer rejected — payee account blocked',
        );
