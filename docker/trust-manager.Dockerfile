...
WORKDIR /app
COPY . .
RUN npm run build:apps-trust-manager
...
#
#   none     KMS-backed profile -- keys are generated in process, no module
...
# no listener: control plane only
CMD ["node", "dist/packages/apps/trust-manager/main.js"]
